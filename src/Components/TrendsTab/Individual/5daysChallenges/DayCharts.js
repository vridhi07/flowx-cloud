...
function DayCharts({
  ...
  const calculateAvgOfSprint = (selectedPhdata,selectedFdData) =>{
    let sprintList = ['sprint 0','sprint 1','sprint 2']
    let allSprintList = []
    if(sprintList?.length > 0 && selectedFdData.length > 0 && selectedPhdata.length > 0 ){
      sprintList.map(val =>{
        let sprintItems = {}
        if(selectedPhdata[0][val] ||  selectedFdData[0][val]){
          let sprint  = []
          if(selectedPhdata[0][val]){
            sprint.push(selectedPhdata[0][val])
          }
          if(selectedFdData[0][val]){
            sprint.push(selectedFdData[0][val])
          }
          if(sprint.length > 1){
              let avgOfSprint = {}
              let flowList = []
              let coheranceList = []
              flowList.push(sprint[0].flowPercent,sprint[1].flowPercent)
              coheranceList.push(sprint[0].coherencePercent,sprint[1].coherencePercent)
             let avgFlow = avgCalculation(flowList)
             let avgCoherance = avgCalculation(coheranceList)
             avgOfSprint.flowPercent = avgFlow
             avgOfSprint.coherencePercent = avgCoherance
             avgOfSprint.sprint = val
             allSprintList.push(avgOfSprint)
            }else{
              let finalSprint =  {...sprint[0] , sprint : val}
              allSprintList.push(finalSprint)
          } 
        }
       
      })
    }
    return allSprintList
  }
            ...
            value:
              ...
              (typeData == "flowData"
                ...
                ? dataList[0]?.program 
                  ...
                  : 0
                : (dataList[0]?.program  && dataList[0][`sprint ${index}`]?.coherencePercent +
                dataList[0]?.program  && dataList[0][`sprint ${index}`]?.flowPercent) /
                  2),
          },
          {
            name: "Group B",
  ...
  const getGraphDayData = (grpahdata, dataList, typeData, colorVariation,kl) => {
    ...
    <div className={removemx0?'row':'row mx-0'}>
      <div className={mobileMediaQuery ?"col-12 col-md-12 col-lg-5 mb-5":"col-12 col-md-12 col-lg-5" }>
        <div className={classes.card}>
          ...
          <div className={mobileMediaGraph?"col-12 col-md-4 col-lg-4":"col-3 col-md-4 col-lg-4"}>
              ...
              <div className="row no-gutter">
                ...
                <div className="col-2">
                <div className={mobileMediaGraph?classes.mobileverticalLine:classes.verticalLine}></div>
                </div>
              </div>
            </div>
            <div className={mobileMediaGraph?"col-12 col-md-8 col-lg-8":"col-9 col-md-8 col-lg-8"}>
